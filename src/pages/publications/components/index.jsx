...
function Publications() {
    ...
    const deleteTable = (id) => {
        ...
        REQUESTS.PUBLICATIONS.DELETE(id, callback, errorCallback);
    };
    ...
    const columns = [
...

        {
            ...
            width: "150px",
            dataIndex: "publication_images",
            render: (record, text, index) => {
                return <img width={100} src={`http://84.21.171.97:3005${record[0]?.path}`} />;
            },
        },
    ...
    const getSandals = () => {
        ...
        REQUESTS.PUBLICATIONS.GET(query, (data) => {
            // setCurrentPage(Number(data?.page));

            setLoading(false);
            setUserSandalsList(data?.message?.rows);
            if (!data == []) {
                setEmpty(false);
            }
        });
